...
describe.skipIf(!process.env.DATABASE_URL)("connections store", () => {
  ...
  it("listConnections shows the added row with hasToken and no token field", async () => {
    ...
    const row = (await listConnections()).find((c) => c.id === added.id);
  ...
  it("setConnectionEnabled flips a connection off and on again", async () => {
    const added = await addConnection({ name: named("toggle"), url: "https://example.com/toggle", transport: "http" });
    await setConnectionEnabled(added.id, false);
    const off = (await listConnections()).find((c) => c.id === added.id);
    expect(off!.enabled).toBe(false);

    await setConnectionEnabled(added.id, true);
    const on = (await listConnections()).find((c) => c.id === added.id);
  ...
  it("listEnabledConnectionsWithSecrets carries the token, which is what the agent's mcpServers needs", async () => {
    ...
    const row = (await listEnabledConnectionsWithSecrets()).find((c) => c.id === added.id);
    expect(row).toBeDefined();
  ...
  it("listEnabledConnectionsWithSecrets leaves out a disabled connection", async () => {
    ...
    await setConnectionEnabled(added.id, false);
    const ids = (await listEnabledConnectionsWithSecrets()).map((c) => c.id);
    expect(ids).not.toContain(added.id);
  });

  it("deletes everything it created", async () => {
    await db.delete(connections).where(like(connections.name, `${PREFIX}%`));
    const left = (await listConnections()).filter((c) => c.name.startsWith(PREFIX));
