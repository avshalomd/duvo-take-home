...
Ten labelled runs in `fixtures/llm-cases.json`, scored on the verdict. **12/12** as expected.
...
| --- | --- | --- | --- | --- |
| eval_good | pass | pass | yes | aQ=0.88 fP=0.8 |
...
| eval_mixed_topic_ambiguous | pass_with_notes | pass_with_notes | yes | aQ=0.77 fP=0.74 - The CSV has the required columns (title, source, url, published_at, summary) and contains 9 rows, meeting the 'at least 8' requirement, with r |
...
| eval_ragged_row | fail | fail | yes | The CSV parses: output.csv: row 6 has 6 fields, the header has 5 |
| eval_url_column_not_urls | fail | fail | yes | The url column holds links: not a url: WION, Reuters |
...
_Generated 2026-09-22T11:02:19.145Z._
