...
| Q232 | review-fe 5 | Details on a phone covers the run, but Tab reaches the hidden controls under it | his call: a modal on phones, the side panel on desktop | minor | fix-final | see fix-final |
...
| Q252 | review-sec 7 | plain members see full connection addresses, which can hold a key | his call: members see the host only; the form warns | minor | fix-final | see fix-final |
| Q253 | review-sec 10, qa-func F18 | Check again and Make an automation are paid calls outside the budget and any rate limit | his call: a cooldown each, and their cost counts toward the day | minor | fix-final | see fix-final |
| Q254 | review-sec 5 | a connection's MCP client may follow a redirect into an internal address (not proven) | recommended: accept and document for now | minor | main | open (his call pending) |
...
| Q265 | qa-ux U3 | a refused Run plays the handover, then snaps back | his call: refused in place, no handover | major | fix-final | see fix-final |
