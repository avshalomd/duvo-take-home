...
| Q139 | final | the Automations gallery is the most default-looking screen: the command breaks at its hyphen, no press feedback | built around the command token and a mini thread; the command never breaks | minor | automations | fixing |
...
| Q142 | final | a chart is labelled "contains 2 payment card numbers" (long decimals pass the card check) | no card warning on decimals or charts | minor | guards | fixing |
...
| Q146 | final | a focused row in the Add a server dialog draws a square ring over the group's rounded corners | the ring follows the shape | minor | settings | fixing |
| Q147 | final | connection names under the composer run together ("DeepWiki e2e Settings moved"); a settings e2e leaves a connection and invitations behind | separated names; tests clean up | minor | home + settings | fixing |
