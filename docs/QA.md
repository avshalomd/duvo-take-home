...
| Q58 | reviewer | the run directory is never removed; on Vercel /tmp accumulates until writes fail | `rm(dir, { recursive: true, force: true })` in the finally; the files are in the DB | minor | engine | open |
