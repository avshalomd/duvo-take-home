...
| Q149 | engine | each attempt's finished event shows the SDK's running total, and the heal prompt repeats the feedback's lead and closing lines | per-attempt cost in Details, one total on the run; no repeated lines | minor | engine + home | engine part fixed (0ae0311: attempt_cost_usd; one framing); Details display with home |
