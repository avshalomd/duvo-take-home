...
| Q84 | reviewer | nothing calls the cron route under RUNNER=inline: a schedule shows a next run that never fires, and `closeAbandonedRuns` never runs, so a run stranded by a restart holds an in-flight slot for ever | abandoned runs closed on the way into a start; the schedule form says when no scheduler is running | major | engine + automations | engine part fixed (d38ea6f, 14b6996: stranded runs closed inside startRun; schedulerRunning()); the schedule form note is with automations |
